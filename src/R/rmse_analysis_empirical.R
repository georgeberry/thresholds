...
ONE_OFF_PATH = "../../data/empirical_replicants/e_N_0_12.0__c_10.0_N_N__empirical_5.0_N_N___American75_gc~12"
RMSE_DF_PATH = "../../data/rmse_df_empirical.csv"
K_DF_PATH = "../../data/k_df_empirical.csv"
...
comparison_df = df[,c('var1', 'threshold_ceil')]
comparison_df$sample = 0
observed_comparison_df = df[df$observed == 1,c('var1','after_activation_alters')]
...
ggplot(k_df, aes(x=k, y=rmse_obs_mean)) +
  geom_smooth(aes(ymin=rmse_obs_mean - rmse_obs_sd, ymax=rmse_obs_mean + rmse_obs_sd)) +
  xlab("k") + ylab("RMSE") +
  theme(text=element_text(size=20)) +
...
ggplot(rmse_df, aes(x=error_sd, y=epsilon_obs_mean)) +
  geom_point() +
...
b = ggplot(rmse_df, aes(x=error_sd, y=beta_obs_mean)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
