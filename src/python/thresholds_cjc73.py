...
def async_simulation(graph_with_thresholds):
    ...
    # could skip this if no nodes are active at start
    for node, data in g.node.iteritems():
        alter_set = set(g.neighbors(node))
        data['active_nbrs'] = len(alter_set & activated_node_set)
        if data['active_nbrs']>=data['threshold']:
            primed_node_set.add(node)
        
#    primed_node_set = [ node for node, data in g.node.iteritems() 
#                        if data['active_nbrs']>=data['threshold']
#                      ]
    ...
    while len(unactivated_node_set) > 0:
        ...
        if g.node[ego]['activated'] == True:
            # Cheap to fail, just skip
            continue
        
        # Only increment if we got a random inactive node
        num_steps += 1

        #alter_set = set(g[ego].keys())
        #activated_alters_num = len(alter_set & activated_node_set)
        threshold = g.node[ego]['threshold']
        activated_alters_num = g.node[ego]['active_nbrs']
        if activated_alters_num >= threshold:
            ...
            steps_without_activation = 0
            for node in g.neighbors(ego):
                g.node[node]['active_nbrs'] += 1
        else:
...
def random_sequence(unode_set):
    r_seq = iter(np.random.choice(list(unode_set),len(unode_set)))
    while True:
        yield r_seq.next()
